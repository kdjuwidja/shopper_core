...
func TestGetShoplistAndItemsForUserByShoplistID(t *testing.T) {
	...
	// Add owner as member to empty shoplist
	emptyShoplistOwnerMember := dbmodel.ShoplistMember{
		ID:         3,
		ShopListID: emptyShoplist.ID,
		MemberID:   owner.ID,
	}
	err = testConn.GetDB().Create(&emptyShoplistOwnerMember).Error
	assert.NoError(t, err)
	...
	// Test case 5: Get empty shoplist and items for owner
	{
		// Create request
		req, _ := http.NewRequest("GET", "/shoplist/2/items", nil)
		req.Header.Set("Authorization", "Bearer test-token")
